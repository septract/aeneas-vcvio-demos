...
- **Demo 4 (message authentication) — `AuthMac.macUF_le`**, the closed UF-CMA bound, backed by
  the real-world correspondence `prfRealExp_reduction_eq`, the random-function forgery bound
  `reduction_RF_le`, and value adequacy `verify_spec_pointwise`. The first **integrity /
  active-adversary** result: a deterministic *canonical* MAC `tag(k,m) = F_k(m)` whose `verify` is
  the **Aeneas-extracted Rust** byte-comparison (`mac.verify`; value adequacy
  ...
  **UF-CMA-secure by a reduction to PRF security**, closed to
  `UF_CMA_Advantage(A) ≤ prfAdvantage(reduction A) + 1/|Tag|` with `1/|Tag| = 2⁻²⁵⁶`. The witness
  ...
  **libsignal HMAC shape** — HMAC is a deterministic canonical PRF-MAC, used in PQXDH's
  ...
  function). The intermediate `macUF_le_prfAdvantage_add_RF` (the open-term form, exactly VCVio's
  own `PRFTagReader.authExp_le_prfAdvantage_add_authRF`) is also retained. **Scope:** constant-time
  comparison is a timing side channel, out of scope.
