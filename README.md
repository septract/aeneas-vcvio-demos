...
>   `sorry`, no `native_decide`, no custom axioms. (One documented exception: the **four** PQXDH
>   key-agreement theorems — `pqxdh_initiate_total`, `pqxdh_accept_total`, and the two
>   `pqxdh_keys_agree_{no,with}_opk` — additionally depend on five *named* hardness-floor axioms (the
>   opaque X25519 / ML-KEM / HKDF / EC-canonicity primitives), and the gate confines those to exactly
>   those four theorems; see `TRUST.md`.) That guarantee rests on Lean + the gate, **not** on
...
|---|---|---|---|
| PQXDH key schedule | `Demos/Pqxdh/KeySchedule.lean` | `pqxdh.rs` (libsignal `pqxdh.rs` + `curve.rs`) | the discontinuity prefix is all-`0xFF`; the 96-byte HKDF output splits to exactly `(root_key, chain_key, pqr_key)` (`derive_arrays`); **`DecodeEC ∘ EncodeEC = id`** (spec §2.1 inverse); the **full HKDF secret-input byte layout** (both paths) `0xFF³² ‖ DH1 ‖ DH2 ‖ DH3 [‖ DH4] ‖ SS`, and the **associated data** `AD = EncodeEC(IK_A) ‖ EncodeEC(IK_B)` — the same domain-separation / transcript byte-layout layer in which the Bhargavan et al. (USENIX'24) re-encapsulation attack arose (caveat: the attack-relevant binding — the KEM public key `PQSPK` in the `AD`/KDF — is **not** modeled here; the modeled secret input carries only the shared secret `SS` and the `AD` is the identity-key binding only. NB this matches **current pinned upstream**: libsignal `pqxdh.rs@5441a83` itself appends only `ss.as_ref()` to the KDF `secret_input` (`pqxdh.rs:226-230`) and binds no kyber pubkey/ciphertext — so this is *faithful to upstream at this commit*, not a lag behind a deployed fix; PQXDH's security against re-encapsulation rests on the KEM being **binding** / IND-CCA, an (A) leaf, not on a `PQSPK`-in-`AD` transcript fix). Also the **full initiator/recipient key agreement** (`pqxdh_initiate`/`pqxdh_accept`): the X25519 agreements, ML-KEM-1024 encaps/decaps, and HKDF are invoked at their faithful **call sites**, so the key→leg wiring (which key produces which DH leg, in which order, into the KDF) is extracted Rust — plus the recipient's `is_canonical` base-key guard. The primitives are `#[charon::opaque]` = five **named hardness-floor axioms** (the gate's one documented exception, see `TRUST.md`); value-adequacy is proved relative to them |
...
| SPQR RS **decoder** | `Demos/Spqr/Gf.lean` | `gf.rs` (SPQR `encoding/polynomial.rs` `lagrange_interpolate`/`compute_at`) | **totality** of the **Lagrange-interpolation reconstruction kernel** (`prepare` ∘ `mult_xdiff` ∘ `complete` ∘ accumulate) + x-power-table evaluation + `decode_value_at`, over fixed `[u16;37]` arrays. The algebraic `decode∘encode = id` round-trip the SCKA correctness argument needs is **partly landed, not all future work**: GF(2¹⁶) irreducibility (`POLY_poly_irreducible`) and the GF16-value-level round-trip (`decode_value_at_roundtrip_gf16_unconditional`) are proved **unconditionally** (`Gf16Irreducible*`/`RsFieldBridge` headlines); the **full field-/Lagrange-level capstone** (`RsCapstone.decode_value_at_roundtrip`) is **registered but conditional** on the Lagrange-interpolation bridge (that the extracted loops compute Mathlib's `Lagrange.interpolate`) + the field-instance assembly — carried as explicit, satisfiable hypotheses, not axioms |
...
what these nodes currently model — which is the same `SS`/identity-key layout **current pinned
upstream `pqxdh.rs@5441a83` itself uses** (it binds no kyber pubkey into the KDF input); so the gap is
shared with upstream, not a lag behind a deployed fix, and re-encapsulation resistance rests on the
KEM being binding/IND-CCA (see `TRUST.md`).
