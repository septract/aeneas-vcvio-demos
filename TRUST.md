...
- **Covers:** every headline theorem (listed in `demos/lean/Audit.lean`) depends transitively on
  ...
  **One documented exception:** the **four** PQXDH key-agreement theorems (`Pqxdh.pqxdh_initiate_total`,
  `Pqxdh.pqxdh_accept_total`, `Pqxdh.pqxdh_keys_agree_no_opk`, `Pqxdh.pqxdh_keys_agree_with_opk`)
  additionally depend on five *named* hardness-floor axioms — the opaque
  ...
  to exactly those four theorems (the `FLOOR_OK` allow-list; any leakage into any other headline fails
  the gate), so the strict 3-axiom guarantee still holds everywhere else. See the PQXDH/SPQR node
  section below. *(The gate's `VERIFY OK` line reports "4 PQXDH orchestration theorem(s)" accordingly.)*
...
>   functional-correctness annotations against `Spec.GF16`). So the mirror has an upstream anchor to
>   check against — but a **partial** one: the upstream F\* spec models
>   (`deps/spqr-src/proofs/fstar/models/Spec.GF16.fst`, `Spec.MLKEM.fst`) contain `admit`/`assume`,
>   i.e. the field-law / ML-KEM layers are *posited*, not fully proved, upstream either. And the
>   SCKA-level meaning of `vulnerable_epochs` is anchored only in the **test harness**
>   (`deps/spqr-src/src/test/messaging_scka.rs`), not a verified property — the real `states.rs:67`
>   `vulnerable_epochs` impl carries no security proof. So "lower risk" is relative (a real codebase
>   anchor exists) but is **not** a machine-checked security guarantee to inherit.
...
| **Hardness floor below the nodes** — X25519/Gap-DH, ML-KEM/Module-LWE (IND-CCA), AES-as-PRP, SHA-256-*compression*-as-PRF/RO | **(A)** | assumed, not extracted — the leaves the eventual reductions bottom out on. In the PQXDH key agreement these are **`#[charon::opaque]` call sites**, which Aeneas emits as five named Lean `axiom`s (`pqxdh.x25519_agree`, `mlkem_encapsulate`, `mlkem_decapsulate`, `hkdf_sha256_derive`, `ec_is_canonical`); `scripts/audit.sh` allows them **only** in the four PQXDH theorems above (the documented gate exception). In the SPQR typestate ML-KEM appears **only as a typed boundary** (a VCVio `KeyEncapMech`), not yet under any security claim. |
| **Floor leaves a *real* PQXDH proof additionally needs (NOT yet among the assumptions)** — KEM **binding** (SH-CR / MAL-BIND-K-CT: distinct ciphertexts/pubkeys cannot collide on the shared secret — the property the BJKS attack defeats), **EUF-CMA** of the signed-prekey signatures, and **KEM-correctness** (decaps recovers encaps) | **(A) — future** | Fiedler–Günther (2024/702) reduces PQXDH key-indistinguishability to GapDH ∧ IND/OW-CCA ∧ these three leaves; the current floor names only IND-CCA. These are not modeled at all today (the signature scheme has no mirror — see next row) and must be added as named (A) leaves when the PQXDH game is built. Recorded so the floor list is not read as complete. |
| **Curve agreement + signatures have NO mirror** (only the EC *byte-encoding* `encode_ec`/`decode_ec` is mirrored/extracted) | **(C)/note** | `demos/rust/pqxdh/pqxdh.rs` mirrors the EC point *serialization* and the key→leg *wiring*, but the X25519 *agreement itself* is the opaque `x25519_agree` axiom (not extracted Rust), and the **prekey-signature verification has no mirror or axiom at all** — upstream verifies signed-prekey signatures in the session layer (`deps/libsignal-src/rust/protocol/src/session.rs`), which the PQXDH node does not model. So EUF-CMA (above) currently has nothing to attach to. A real proof must either mirror the signature-verification wrapper or carry prekey-signature EUF-CMA as an explicit (A) leaf. |
