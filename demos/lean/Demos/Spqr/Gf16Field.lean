/-
  ...
  precise, honest obligation rather than papered over with a cheat:
    ...
    (B-irr)  `Irreducible POLY_poly` over `ZMod 2`. Mathlib has NO usable decision
             procedure for polynomial irreducibility over `ZMod 2`; `decide`/`native_decide`
             is forbidden by the task rules AND infeasible at degree 16; the honest route
             is a Rabin-style factor-exclusion (≈511 candidate divisors of degree ≤ 8),
